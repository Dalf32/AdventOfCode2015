...
class Location
	...
	def find_shortest_path_recursive(locations, visited_locations)
		...
		locations_left.map{ |loc| locations[loc].find_shortest_path_recursive(locations, visited_locations + [loc]) + @distances[loc] }.min
	end
end
...
puts locations.values.map{ |location| location.find_shortest_path(locations) }.min
